...
class FinalPriceBox
{
    ...
    {
        $saleableItem = $finalPriceBox->getSaleableItem();
        $customAttribute = $saleableItem->getCustomAttribute('show_price');
        $show_price = $customAttribute ? $customAttribute->getValue() : null;
        $ob = \Magento\Framework\App\ObjectManager::getInstance();
        $customerSession = $ob->create("Magento\Customer\Model\Session");
        ...
        if($show_price != true){
            if (strstr($finalPriceBox->getTemplate(), 'final_price.phtml') ||
                ...
                $collection=$saleableItem->getCollection()->addAttributeToSelect('*')->addAttributeToFilter('show_price', 1);
                foreach ($collection as $value) {
                    if($saleableItem->getSku() == $value->getSku() ){
                        return $result;
                    }
                }
