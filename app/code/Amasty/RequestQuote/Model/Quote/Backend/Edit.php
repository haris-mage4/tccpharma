...
class Edit extends DataObject
{
    ...
    public function setAccountData($accountData)
    {
        ...
        foreach ($form->getAttributes() as $attribute) {
            if ($attribute->getAttributeCode() == "facility") {
                $code = sprintf('customer_%s', 'facility_name');
                $data[$code] =  $customerData['facility_name'] ?? null;
            } else if($attribute->getAttributeCode() == "select_sales_rep"){
                $code = sprintf('customer_%s', 'sales_representative');
                $data[$code] =  $customerData['sales_representative'] ?? null;
            } else {
                $code = sprintf('customer_%s', $form->getAttributes());
                $data[$code] =  $customerData[$form->getAttributes()] ?? null;
            }
        }
    ...
    public function updateAccountData($accountData)
    {
        ...
        if ($accountData['facility']) {
            $customer->setData('facility_name', $accountData['facility']);
        }
        if ($accountData['select_sales_rep']) {
            $salesRep = $accountData['select_sales_rep'];
            switch ($salesRep) {
                case '18':
                    $salesRep = '41';
                    break;
            ...
            $customer->setData('sales_representative', $salesRep);
        }
        if ($accountData['facility'] || $accountData['select_sales_rep']) {
            $customer->save();
            return true;
        }
        return false;
    ...
    public function importPostData($data)
    {
        if (is_array($data)) {
            ...
            if (isset($data['quote_entity'])) {
                $this->updateAccountData($data['quote_entity']);
            }
