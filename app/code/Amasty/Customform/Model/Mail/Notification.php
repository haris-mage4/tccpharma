...
class Notification
{
    ...
    private function sendAdminNotification(Form $formModel, Answer $model)
    {
        ...
        if ($emailTo && $this->isSendNotification($formModel)) {
            ...
            $form_data=json_decode($model->getResponseJson(),true);
            $key = array_column($form_data, 'label');
            $searchValue = 'Name';
            $result = array_filter($form_data, function ($item) use ($searchValue) {
                return $item['label'] === $searchValue;
            });
            if (!empty($result)) {
                $parentKey = key($result);
                $guest_customer_name=$form_data[$parentKey]['value'];
            } else {
                $guest_customer_name='Guest';
            }
            $customer_name = (is_string($customerData['customer_name'])) ?$customerData['customer_name']: $guest_customer_name;
            try {
