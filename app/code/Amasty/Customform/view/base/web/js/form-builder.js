    ...
    $.widget('mage.customFormBuilder', {
        ...
        addCollapseDefaultValueVariables: function (element) {
            $(element).find('.value-wrap .default-values-link').on('click', function (e) {
                var variantsBlock = $(e.target).closest('.input-wrap').find('.default-value-variants');
        ...
        getNote: function (attribute, values) {
            ...
            if (attribute === 'value' && allowTypes.includes(values.type)) {
                note = '<a class="default-values-link" href="#">' + this.options.messages['notes'][attribute]['label'] + '</a>';

                this.options.messages['notes'][attribute]['values'].forEach(function (value) {
                    variables += '<span class="variable">' + value + '</span>'
                });
                note += '<div class="default-value-variants">' + variables + '</div>';
            }
