...
class ConvertToXml extends \Magento\Ui\Model\Export\ConvertToXml
{
    ...
    protected function getFields()
    {
        if (!$this->fields) {
            ...
            //     array_push($this->fields,"order_item");
            // }
        }
        return $this->fields;
    }
    ...
    protected function prepareItems($componentName, array $items = [])
    {
        foreach ($items as $document) {
            ...
            // if($componentName=='amasty_quote_grid') {
                
            //     $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            //     $quoteData = $objectManager->create('Magento\Quote\Model\Quote')->load($document->getEntityId());
            ...
            //             $itemData[] = "Product Name : ".$product->getName();
            //             $itemData[] = "Sku : ".$quoteItem->getSku();
            //             $itemData[] = "Qty : ".$quoteItem->getQty();
            //             $itemData[] = "Price : ".$quoteItem->getPrice();
            //             $itemData[] = "Discount Amount : ".$quoteItem->getDiscountAmount();
            //             $itemData[] = "NDC : ".$quoteItem->getNdc();

            //     }
            //     $exportItemData = implode(" | ", $itemData);
            //     $document->setOrderItem($exportItemData);
            // }
