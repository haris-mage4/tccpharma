 ...
 */
class MassCrossSell extends AbstractLinkProduct
{
    /**
    ...
    public function massAction($collection)
    {
        ...
        $data    = $request->getPost('cross_sell');
    ...
    {
        $currentSku = [];
        /** @var Collection $currentLinkProdCol */
        if ($currentLinkProdCol = $product->getCrossSellProductCollection()) {
    ...
    public function getProductLinkData($newSkuLinks, $product)
    {
        ...
        if ($oldLinks = $product->getProductLinks()) {
            foreach ($oldLinks as $oldLink) {
                if ($oldLink->getLinkType() !== 'crosssell') {
                    $linkDataAll[] = $oldLink;
                }
            }
        }
