 ...
 */
class RemoveWebsite extends AbstractElement
{
    /**
     * @var HelperData
     */
    ...
    {
        $html = '<div class="messages"><div class="message message-notice">';
        $html .= '<div>' . __('To hide an item in catalog or search results, set the status to "Disabled".') . '</div>';
        $html .= '</div></div>';
        $html .= '<div class="store-scope"><div class="store-tree" id="remove-products-to-website-content">';
        foreach ($this->_helperData->getWebsiteCollection() as $_website) {
            /** @var Website $_website */
            $html .= '<div class="website-name">';
            $html .= '<input name="product[remove_website_ids][]" value="' . $_website->getId() . '" 
            class="checkbox website-checkbox" 
            id="remove_product_website_' . $_website->getId() . '" type="checkbox" />';
            $html .= '<label for="remove_product_website_' . $_website->getId() . '">' .
                $_website->getName() . '</label>';
            $html .= '</div>';
            $html .= '<dl class="website-groups" id="remove_product_website_' . $_website->getId() . '_data">';
