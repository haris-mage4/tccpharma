...
namespace Mageplaza\MassProductActions\Block\Adminhtml\UpSell\Edit;
...
/**
 * Class Form
 * @package Mageplaza\MassProductActions\Block\Adminhtml\UpSell\Edit
 */
class Form extends Generic
{
    ...
    protected function _prepareForm()
    {
        ...
        $form = $this->_formFactory->create([
            'data' => [
                'id'      => 'mp_up_sell_edit_form',
                'action'  => $this->getUrl(
                    'mpmassproductactions/product/massUpSell',
        ...
        $form->setHtmlIdPrefix('up_sell_');
        $form->setFieldNameSuffix('up_sell');

        $onclickText = 'mpMassProductAction.loadProductsGrid(event,mpUpSellProductsGridUrl);this.hide();';
        $fieldset    = $form->addFieldset('base_fieldset', ['class' => 'fieldset-wide']);

        $fieldset->addField('action', 'select', [
            'label'  => __('Remove Up-Sell Products'),
            'title'  => __('Remove Up-Sell Products'),
            'name'   => 'action',
        ...
        $fieldset->addField('up_sell', 'hidden', [
            'name' => 'up_sell',
        ]);

        $fieldset->addField('add_products', 'text', [
            'name'        => 'add_products',
            'label'       => __('Add Up-Sell Product(s)'),
            'title'       => __('Add Up-Sell Product(s)'),
            'class'       => 'mp_products_input_text',
            'placeholder' => 'product SKU',
            'note'        => $this->_helperData->getSelectProductHtml($onclickText)
                . $this->_helperData->getSubmitProductHtml($this->getViewFileUrl('images/loader-1.gif'))
        ...
        $this->setChild(
            ...
            $this->getLayout()->createBlock(Dependence::class)
                ->addFieldMap('up_sell_action', 'action')
                ->addFieldMap('up_sell_remove_products', 'remove_products')
